...
=============================================================================
A) "physical" — สระ/ฝาย ทุกจังหวัด + อ่างเก็บน้ำนอกภาคเหนือ
...
=============================================================================
สมมติฐานที่ยังไม่ได้ยืนยันกับผู้ใช้โดยตรง (ทำไปตามคำสั่ง "ลงมือทำทั้งหมดได้เลย" — ไม่ fabricate ค่าลอยๆ
แต่ตัดสินใจเลือกวิธีที่อนุรักษ์นิยม/สมเหตุสมผลที่สุดแล้วบันทึกไว้ตรงนี้ให้ตรวจสอบทีหลังได้)
=============================================================================
1. แบ่งสัดส่วน "น้ำท่าเข้า" (inflow) ให้แหล่งน้ำที่อยู่กลุ่มเดียวกัน (หมู่บ้านเดียวกัน หรือทั้งตำบลถ้าเป็น
   ...
   ลุ่มน้ำมาก)
2. แบ่งสัดส่วน "น้ำที่ถูกดึงไปใช้" (outflow) — **อัปเดต 2569-08-25: พบว่าตาราง reservoir_village_usage มีอยู่
   แล้วในฐานข้อมูลจริง (13 แถว เก็บ households/irrigated_area_rai ต่อคู่แหล่งน้ำ×หมู่บ้านของอ่างเก็บน้ำ 5 แห่ง)
   จึงใช้ข้อมูลจริงนี้เป็นน้ำหนักหลักแทนการเดาแล้ว** — households/irrigated_area_rai คนละหน่วยกัน (ครัวเรือน vs
   ไร่) เอามารวมกันตรงๆ เป็น "น้ำหนักสัมพัทธ์" คร่าวๆ ไม่ใช่ค่าจริงเชิงฟิสิกส์ (ยังเป็นการประมาณอยู่ดี แค่แม่นกว่า
   เดิม) เฉพาะแหล่งที่ไม่มีแถวใน reservoir_village_usage เลย จึง fallback ไปถ่วงน้ำหนักตามสัดส่วน
   stored_capacity_m3 ในหมู่บ้านเดียวกันแทน (สมมติฐานเดิม เป็นสมมติฐานเดียวกับที่ balance_engine.py ใช้อยู่แล้ว
   ตอนรวม pond_stock) — **ทั้งสองวิธียังไม่เคยเสนอให้ผู้ใช้ยืนยันสูตรการถ่วงน้ำหนักเป็นการเฉพาะ ควรตรวจสอบก่อนใช้
   ผลลัพธ์ระดับปฏิบัติการจริง**
...
1. capture_efficiency เป็นค่าคงที่ต่อประเภทแหล่งน้ำ (ไม่ผันตามฤดูกาล/สภาพจริงของแต่ละแหล่ง) — ยืนยันกับ
   ผู้ใช้แล้ว 2569-08-25: อ่างเก็บน้ำ > 50% (ใช้ 55%), สระ ≤ 15% (ใช้ 15%), ฝาย = 2%
...
3. small_water_source: ยังไม่มีค่า capture_efficiency ที่ยืนยัน — แหล่งประเภทนี้จึงถูกข้ามไปเงียบๆ ไม่มีแถวใน
   storage_depletion_monthly เลย (ดู CAPTURE_EFFICIENCY_BY_SOURCE_TYPE ไม่มีคีย์นี้)
4. groundwater_well/mountain_spring/purchased_external: ไม่มีแนวคิด "น้ำท่าไหลเข้า" ที่ใช้ได้ (น้ำบาดาล/
   น้ำพุ/น้ำซื้อจากภายนอก ไม่ได้มาจากน้ำท่าในพื้นที่) จึงไม่มีโมเดลรองรับเลยในฟีเจอร์นี้ ต้องออกแบบแยกต่างหาก
   ถ้าต้องการ
5. stored_capacity_m3 เป็น NULL — ข้ามแหล่งนั้นไปทั้งหมด (ไม่มีทาง bootstrap % หรือคลิป overflow ได้เลย)
6. **ฝายทุกแห่งในระบบ (53/53) ไม่มี stored_capacity_m3 เลยสักแห่ง** (ตรวจสอบจริงหลังรันครั้งแรก 2569-08-25) —
   ผลคือ <code>storage_depletion_monthly</code> ไม่มีแถวของฝายเลยแม้แต่แถวเดียวในทางปฏิบัติ (0 แถว แม้จะตั้ง
   capture_efficiency=2% ไว้แล้วก็ตาม เพราะติดข้อจำกัดข้อ 5 ด้านบนก่อนถึงจุดที่จะใช้ค่านี้) — **ยืนยันจากผู้ใช้
